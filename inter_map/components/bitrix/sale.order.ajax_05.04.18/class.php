...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function getLocationHtml($property, $locationTemplate)
	{
		...
		$debug['res1'] = [
			'PropertyCollection'                     => $this->order->getPropertyCollection()->getArray(),
			'property'                     => $property,
			'location'                     => $location->getAllFields(),
			'locationTemplate'             => $locationTemplate,
			'propertyId'                   => $propertyId,
			'locationAltPropDisplayManual' => $locationAltPropDisplayManual,
			'altPropManual'                => $altPropManual,
		...
		if (!empty($actualValues) && is_array($actualValues))
		{
			foreach ($actualValues as $key => $value)
			{
				...
				// TODO // хак - из basket в order передается defaultCity - brn, только в этом случае нужно править
				// in /include/session.php
				if (count ($actualValues) == 1 && $property['IS_LOCATION'] == 'Y'
					&& $_SESSION['basket'] && $_SESSION['basket']['locale']
					&& ! ($_POST && $_POST['order'] && $_POST['order']['ORDER_PROP_2'])
				) {
//					в первый раз, когда приходит из basket
					$parameters['ID'] = $_SESSION['basket']['locale'];

					$debug[$debug['i']]['id'] = $_SESSION['basket']['locale'];
					$debug[$debug['i']]['order'] = $_POST['order'];
				} elseif($_POST['order']
					&& $_POST['order']['RECENT_DELIVERY_VALUE'] == $_SESSION['basket']['locale']
					&& $_POST['order']['ORDER_PROP_2'] != $_POST['order']['RECENT_DELIVERY_VALUE']
				){
//					когда изменяется город
					$_SESSION['basket']['locale'] = $_POST['order']['ORDER_PROP_2'];
					$_SESSION['basket']['street'] = $_POST['order']['street'];
					$_SESSION['basket']['housing'] = $_POST['order']['housing'];
				}
				...
				$debug[$debug['i']]['beforeIC'] = [
					'template' => 'bitrix:sale.location.selector.'.$locationTemplate,
				...
				$showDefault = false;
				$debug['steps'][$debug['i']] = $parameters;
...
//		\Adelfo\Utils\Log::addMessageToLog('saleOrderAjaxComp_local_getLocationHtml',[
//			'_POST' => $_POST,
//			'_GET' => $_GET,
//			'_SESSION' => $_SESSION,
//			'debug' => $debug,
