...
$typeExchange = "";
if (isset($_GET["base"])) $typeExchange = $_GET["base"];
...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	...
	$importParameters = [
		...
		"table_name" => CBitrixCatalogImport1C::getTableName($typeExchange)
	];

	if ($NS["STEP"] < 1)
	{
		$obXMLFile = new CIBlockXMLFile(CBitrixCatalogImport1C::getTableName($typeExchange));
	...
	elseif ($NS["STEP"] == 2) {
        if ($typeExchange == 'discount') {
            $ob = new Trofey\Tools\Discount\ImportDiscount($ABS_FILE_NAME, CBitrixCatalogImport1C::getTableName($typeExchange));
            $discountResult = $ob->processImport();
            $strMessage = "Обмен правилами корзины завершен. Добавлено ".$discountResult['ADD'].", изменено ".$discountResult['UPDATE'].", удалено ".$discountResult['DELETE']." правил.";
            if (count($discountResult['ERRORS'])>0) {
                $strMessage .= " Ошибки:";
                foreach ($discountResult['ERRORS'] as $error) $strMessage .= $error.',';
            }
            $NS["STEP"] = 3;
        } else {
	...
	else
	{
	    ...
	    $exchangeType = '';
	    if (strlen($_GET['filename'])>6) $exchangeType = substr($_GET['filename'], 0, 6);
	    if (\Bitrix\Main\Loader::includeModule("trofey.tools")&&(($exchangeType=='import')||($exchangeType=='offers'))) {
		$resMarking = \Trofey\Tools\Marking\Base::processMarking();
		$resSize = \Trofey\Tools\Catalog\Base::syncSizeAllProducts();
	      }
	}
}
//Final step
